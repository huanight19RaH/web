...
from scipy.special import expit # Used for ONNX Sigmoid function
import onnxruntime as ort # ONNX inference library
import requests
...
# --- 2. CORE FUNCTIONS & DATA PROCESSING ---
@st.cache_data
...
@st.cache_data(show_spinner="Extracting global data overview...")
...
def load_flood_model(model_path):
    """Load ONNX Model and check for CUDA GPU"""
    try:
        # Configure priority for CUDA, fallback to CPU
        providers = ['CUDAExecutionProvider', 'CPUExecutionProvider']
        session = ort.InferenceSession(model_path, providers=providers)
        
        # Check if ONNX Runtime actually detects CUDA
...
def predict_flood(session, input_name, matrix_x, threshold=0.01, fast_eval=False): 
    ...
    # --- FIX NODATA ERROR: Lower Threshold and keep base 0 ---
...
def display_interactive_prediction_map(prediction, key_suffix):
    if prediction.max() == 0:
        st.info("🟢 The model predicts NO FLOODING for this date's data (all pixels = 0).")
        return 
    ...
    if map_output and map_output.get("last_clicked"):
        ...
        if lat_min <= lat <= lat_max and lon_min <= lng <= lon_max:
            ...
            if val > 0:
                st.error(f"📍 Coordinates: {lat:.4f}, {lng:.4f} | 🌊 **Flood probability: {val:.4f}**")
            else:
                st.success(f"📍 Coordinates: {lat:.4f}, {lng:.4f} | 🟢 **Safe zone (No flooding)**")
        else:
            st.warning("⚠️ You clicked outside the Da Nang area!")

# --- 4. MAIN INTERFACE ---
def main():
    from pathlib import Path
        
    # Use pathlib to get the exact directory containing app.py regardless of the machine
    BASE_DIR = Path(__file__).parent.resolve()   

    DATA_VIZ_DIR = str(BASE_DIR / "Data_Visualize")
    DATA_TRAIN_DIR = str(BASE_DIR / "Data_Training_2")
    ...
    if not os.path.exists(DATA_VIZ_DIR) or not os.path.exists(DATA_TRAIN_DIR):
            st.error(f"⚠️ Missing data folders! Please ensure the following exist:\n- {DATA_VIZ_DIR}\n- {DATA_TRAIN_DIR}")
            return
        
    all_train_files = sorted(glob.glob(os.path.join(DATA_TRAIN_DIR, "Sample_*.npz")))
    if not all_train_files:
        st.warning("No data found in Data_Training")
        return
    ...
    # Initialize model at top level to check GPU
    ...
    with st.sidebar:
        ...
        if provider == "CUDAExecutionProvider":
            st.success("🟢 CUDA GPU: Active (Ultra-fast)")
        else:
            st.warning("🟠 CPU: Running on CPU (ONNX could not find CUDA)")
            
        st.markdown("---")
        
        selected_display = st.selectbox("📅 Select observation date", display_dates)
        ...
        if X_viz is None or X_train is None:
            st.error(f"Error: Data for date {selected_date} is missing in Data_Visualize or Data_Training!")
            st.stop()
        
        if "TEST" in selected_display:
            st.info("✅ Can run AI Prediction.")
        else:
            st.warning("⚠️ AI Prediction feature is locked.")

        st.markdown("---")
        opacity = st.slider("Map Opacity", 0.0, 1.0, 0.6)
    ...
    # ==========================================
    with tab_map:
        st.subheader("Multi-layer Spatial Map & Pixel Query")
        col_map, col_info = st.columns([3, 1])
        
        with col_map:
            selected_lyr = st.selectbox("Select Layer to display on map:", range(8), format_func=lambda x: LAYER_CONFIG[x]["name"])
            m = folium.Map(location=DANANG_CENTER, zoom_start=11, tiles="CartoDB dark_matter")
        ...
        with col_info:
            ...
            st.caption("Click on the map to view data.")
            if map_output and map_output.get("last_clicked"):
                lat = map_output["last_clicked"]["lat"]
                lng = map_output["last_clicked"]["lng"]
                lat_min, lon_min, lat_max, lon_max = 15.95, 107.90, 16.25, 108.40
                ...
                if lat_min <= lat <= lat_max and lon_min <= lng <= lon_max:
                    ...
                    st.markdown("---")
                    st.error(f"🎯 **Ground Truth Label (Y): {Y_viz[r_idx, c_idx]:.4f}**")
                else:
                    st.warning("⚠️ You clicked outside the data range!")
    ...
    with tab_viz:
        ...
        eda_global, eda_daily = st.tabs(["🌍 GLOBAL OVERVIEW", "📅 DAILY DETAILS"])
        ...
        with eda_global:
            ...
            if not df_global.empty:
                ...
                with g_col1:
                    ...
                    st.pyplot(fig_gcorr)
                with g_col2:
                    st.markdown("**2. Distribution (Boxplot)**")
    ...
    # ==========================================
    with tab_predict:
        st.subheader("🤖 Model Evaluation: ONNX Runtime (Quantized INT8)")
        tab_daily_pred, tab_global_eval = st.tabs(["📅 Daily Evaluation", "🌍 Global Evaluation"])
        
        with tab_daily_pred:
            if "TEST" not in selected_display:
                st.error("🚫 Select a **🟢 [TEST]** date in the Sidebar to evaluate.")
            else:
                if st.button("🚀 Run Inference for this Date", type="primary"):
                    if session:
                        with st.spinner("Running ultra-fast Neural Network via ONNX..."):
                ...
                if 'current_prediction' in st.session_state:
                    ...
                    st.markdown("---")
                    st.markdown("### 📊 Hydrological Evaluation Metrics")
            ...
            if st.button("🔄 Start Global Evaluation"):
                ...
                if not test_files:
                    ...
                    for idx, file_path in enumerate(test_files):
                        ...
                        status_text.text(f"Processing {idx+1}/{len(test_files)}: {file_name}")
                        try:
    ...
    # ==========================================
    with tab_live:
        st.subheader("📡 Weather Updates & Real-time Prediction")
        ...
        with col_input:
            ...
            simulation_mode = st.radio("Mode:", ["🌤️ Real API", "⛈️ Storm Simulation"])
        with col_action:
            st.write("\n\n")
            run_live = st.button("⚡ Run Prediction", type="primary", use_container_width=True)
        ...
        if run_live:
            ...
            with st.spinner("📦 Extracting static terrain data..."):
            ...
            with st.spinner("🌩️ Calling Weather API & IDW Interpolation..."):
                api_results = []
                is_flood_test = "Simulation" in simulation_mode
            ...
            with st.spinner("🤖 Running Inference via ONNX..."):
                if session:
